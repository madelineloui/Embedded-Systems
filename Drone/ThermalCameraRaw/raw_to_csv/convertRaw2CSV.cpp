...
void onThermalFrame(unsigned short* thermal, unsigned int w, unsigned int h, IRFrameMetadata meta, void* arg)
{
  ...
  // CSV output file name = input filename with appendix {counter} and ending "csv"
  char* csvFileName = new char[len+7];
  memcpy(csvFileName, _fileIn, len);
  sprintf(&(csvFileName[len-4]), "_%05d.csv", _cnt);
  csvFileName[len+6] = '\0';
  cout << "creating " << csvFileName << "\n";

  // Configuration of image conversion. Uncomment this section to generate PPM files
  ...
  // generate csv file with temperature in celsius at each pixel
  ofstream csv(csvFileName);
  int count = 0;
  for(int x = 0; x < h; x = x + 1) {
  	for(int y = 0; y < w; y = y + 1) {
    	    	float t = ((((float)thermal[count])-1000.f))/10.f;
		count = count + 1;
	    	csv << t << ",";
	}
	csv << "\n";
